...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <div class="vx_wrap"> 
  <div>
  <h2  class="vx_img_head"><img alt="<?php _e("Mailchimp Feeds", 'contact-form-mailchimp-crm') ?>" title="<?php _e("Mailchimp Feeds", 'contact-form-mailchimp-crm') ?>" src="<?php echo $this->get_base_url()?>images/mailchimp-crm-logo.png?ver=1" /> <?php _e("Mailchimp Log", 'contact-form-mailchimp-crm'); ?>
  
   <select style="min-width: 120px; max-width: 200px; margin-left: 12px; font-weight: normal;" id="vx_sel_feed">
  <option value=""><?php _e('All Feeds','contact-form-mailchimp-crm') ?></option>
  <?php    
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php _e('Search','contact-form-mailchimp-crm') ?>" value="<?php echo $this->post('search') ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php _e('All Objects','contact-form-mailchimp-crm') ?></option>
  <?php    
  ...
  <option value=""><?php _e('All Status','contact-form-mailchimp-crm') ?></option>
  <?php
  ...
  <option value=""><?php _e('All Times','contact-form-mailchimp-crm') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php _e('From Date','contact-form-mailchimp-crm') ?>From Date" value="<?php if(isset($_REQUEST['start_date'])){echo $this->post('start_date');}?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php if(isset($_REQUEST['end_date'])){echo $this->post('end_date');}?>" placeholder="<?php _e('To Date','contact-form-mailchimp-crm') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" title="<?php _e('Search','contact-form-mailchimp-crm') ?>" name="search" class="button-secondary button crm_input_inline"><i class="fa fa-search"></i> <?php _e('Search','contact-form-mailchimp-crm') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e('Apply','contact-form-mailchimp-crm') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php _e('Apply','contact-form-mailchimp-crm') ?></button>
  ...
  ?>
  <button type="button" name="tab_action" title="<?php _e('Export as CSV','contact-form-mailchimp-crm') ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php _e('Export as CSV','contact-form-mailchimp-crm') ?></button> 
  <?php
  }
  ...
  if(isset($data['feeds'][0]['entry_id']) && $data['feeds'][0]['entry_id']!=""){
     $entry_id=$data['feeds'][0]['entry_id'];
      ?>
  <a href="<?php echo   $log_link.'&entry_id='.$entry_id;?>" title="<?php echo sprintf(__('View Entry# %s Logs','contact-form-mailchimp-crm'),$entry_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf(__('View Entry# %s Logs','contact-form-mailchimp-crm'),$entry_id); ?></a><?php
  }}
 if($this->post('entry_id') !="" || $_log_id !=""){
          ?><a href="<?php echo $log_link;?>" title="<?php _e('View All Logs','contact-form-mailchimp-crm') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php _e('View All Logs','contact-form-mailchimp-crm') ?></a>        
          <?php
      }
...
if($items>0){
  ?>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php _e('Displaying','contact-form-mailchimp-crm') ?> <span id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
 <?php
}
        ?>       
  </div>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php _e("Mailchimp ID", 'contact-form-mailchimp-crm') ?>
  <i class="fa fa-caret-<?php echo $crm_order ?> vx_sort_icon <?php echo $crm_class ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="entry_id"><?php _e("Entry ID", 'contact-form-mailchimp-crm') ?>
  <i class="fa fa-caret-<?php echo $entry_order ?> vx_sort_icon <?php echo $entry_class ?>"></i>                      
  </th>
  <th scope="col"><?php _e('Feed ID', 'contact-form-mailchimp-crm') ?> </th>
  <th scope="col" class="manage-column"  data-name="object"><?php _e("Description", 'contact-form-mailchimp-crm') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php _e("Time", 'contact-form-mailchimp-crm') ?>
  <i class="fa fa-caret-<?php echo $time_order ?> vx_sort_icon <?php echo $time_class ?>"></i>
  </th>
  <th style="width: 40px"><?php _e('Detail','contact-form-mailchimp-crm') ?></th>
  </tr>
  </thead>
  ...
  <th><?php _e('Detail','contact-form-mailchimp-crm') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo $row['id'] ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo $base_url ?>images/<?php echo $row["status_img"] ?>.png" alt="<?php echo $row["status"] ? __("Active", 'contact-form-mailchimp-crm') : __("Inactive", 'contact-form-mailchimp-crm');?>" title="<?php echo $row['title'];?>" class="crm_status_img" /></td>
...
$feed_link=$this->get_feed_link($row['feed_id'],$row['form_id']);
 ?><a href="<?php echo $feed_link ?>" title="<?php _e('Edit Feed','contact-form-mailchimp-crm') ?>">#<?php echo $row['feed_id'] ?></a><?php  } ?>
 </td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php _e('Expand Details','contact-form-mailchimp-crm') ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php _e("No Record(s) Found", 'contact-form-mailchimp-crm'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php _e('Clear Mailchimp Log','contact-form-mailchimp-crm') ?>" href="<?php echo wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce'); ?>"><?php _e('Clear Mailchimp Log','contact-form-mailchimp-crm') ?></a>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php _e('Displaying','contact-form-mailchimp-crm') ?> <span id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php _e('Please Select Action','contact-form-mailchimp-crm') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php _e('Please select at least one entry','contact-form-mailchimp-crm') ?>');
    return false;
}
var action=sel.val();
 ...
 if(!confirm('<?php _e('Exporting more than 4 entries may take too long.\\n Are you sure you want to continue?','contact-form-mailchimp-crm') ?>')){
  e.preventDefault();    
   ...
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php _e('Mailchimp Logs will be deleted permanently. Do you want to continue?','contact-form-mailchimp-crm') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(td.is(":visible"))
    {
      next_tr.find('.vxa_entry').slideUp('fast',function(){
       ...
       if(is_main){
    icon.attr('title','<?php _e('Expand Detail','contact-form-mailchimp-crm'); ?>');    
    }        
     return;   
    }else{
     next_tr.show('fast'); 
    ...
    if(is_main){
    icon.attr('title','<?php _e('Collapse Detail','contact-form-mailchimp-crm'); ?>');    
    }    
    }
